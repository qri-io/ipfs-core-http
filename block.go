...
// it's often preferrable to create a CoreHTTP & call coreapi.Block() to get a
// BlockAPI instance
...
func (c BlockAPI) Put(ctx context.Context, src io.Reader, opts ...caopts.BlockPutOption) (coreiface.BlockStat, error) {
	...
	w, err := writer.CreateFormFile("file", "file.txt")
	if err != nil {
		return nil, err
	}
	...
	req, err := http.NewRequest("POST", fmt.Sprintf("%s/block/put", c.url), body)
	if err != nil {
		return nil, err
	}
